...
if ss.chosen_topic:
    ...
    if submit_exemplar or submit_no_exemplar:

        if submit_exemplar:
            prompt = modify_prompt(exemplar=True)
        else:
            prompt =  modify_prompt(exemplar=False)
        ...
        if ss.sub == 'guest_cerith':
            save_ai_response(submitted_text,paragraph['topic'],response, model=st.session_state['openai_model'], prompt=prompt)