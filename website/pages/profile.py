...
if check_user_attribute() and (st.user.given_name or st.user.given_name != ''):
    ...
    st.title(f'Hey, {guest_username}! 👋')
else:
    st.title(f'Hey there! 👋')

# list of indices of the topics already learnt by user
learnt_topics = list(set(load_user_topics('website/data/user_learnt_topics.csv')))
...
if len(checked_boxes) > 0:
    ...
    save_user_topics(checked_boxes, 'website/data/user_learnt_topics.csv')
