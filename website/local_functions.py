...
def load_dictionary():
    ...
    for char in divisions:
        ...
        st.markdown(f'## {get_key_from_value(char, letter_sort)}')
...
def get_word_details(searchterm):
    df = pd.read_csv(filepath_or_buffer='website/data/eurfa_dictionary/simple_dictionary.csv', index_col=0)
...
def show_searched_word(row):
    ...
    abbs = {record['Abbreviation']:record['Meaning (English)'] for record in pd.read_csv('website/data/eurfa_dictionary/abbreviations.csv').to_dict('records')}
...
def save_user_topics(topics, filepath):
    ...
    if user_id not in df['user_id'].to_list():
